...
export const defaultSvgPath = ({ size, position, borderRadius: radius, }) => {
    if (radius) {
        return `M${position.x},${position.y}H${position.x + size.x} a${radius},${radius} 0 0 1 ${radius},${radius}V${position.y + size.y - radius} a${radius},${radius} 0 0 1 -${radius},${radius}H${position.x} a${radius},${radius} 0 0 1 -${radius},-${radius}V${position.y + radius} a${radius},${radius} 0 0 1 ${radius},-${radius}Z`;
    }
...
const getInterpolator = memoize((previousPath, shape, position, size, maskOffset = 0, borderRadius = 0) => {
    const options = {
...
export const svgMaskPathMorph = ({ previousPath, animation, to: { position, size, shape, maskOffset, borderRadius }, }) => {
    const interpolator = getInterpolator(cleanPath(previousPath), shape, position, size, maskOffset, borderRadius);
